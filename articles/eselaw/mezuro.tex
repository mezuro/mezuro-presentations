...
\section{Arquitetura}
  ...
  Com o projeto inteiramente reescrito, evoluímos a arquitetura para o estado atual na figura \ref{fig:architecture-2} que representa as mesmas operações da figura anterior.
  ...
  Explorando mais a fundo a figura \ref{fig:architecture-2}, notamos que a separação do serviço monolítico em dois serviços menores e especializados, Kalibro Processor e Kalibro Configurations, produziu mais complexidade do ponto de vista da quantidade de mensagens trocadas, trazendo a impressão de piorar a manutenabilidade do projeto. Por outro lado, dada uma interface muito bem definida de comunicação como foi feita antes da refatoração do serviço, o aumento na troca de mensagens gera impactos mínimos que são compensados pela modularização e maior especialização dos serviços.
