...
\end{itemize}
  ...
  A seguir apresentaremos ilustradas por meio de capturas de telas, as duas principais funcionalidades da ferramenta. Sempre supondo que já se está em uma conta cadastrada no sistema (único privilégio necessário para realizá-las).
    ...
    \begin{itemize}
      \item \textbf{Aggregation form:} Por exemplo, dada uma pasta com diversos arquivos contidos, como será calculada a nota desta pasta (média das notas dos arquivos entre outras formas disponíveis);
      \item \textbf{ReadingGroup:} Conjunto de intervalos usado na interpretação da nota dada, sendo o que dá um significado prático à nota calculada.
    \end{itemize}
