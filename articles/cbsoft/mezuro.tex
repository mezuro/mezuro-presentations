...
\begin{abstract}
  In this article we present the motivation, main caracteristics, architecture and examples of use of the free software project Mezuro.
  The motivation behind the project will be presented through the problem of analyzing code metric, making a brief comparison with 
  other solutions avaliable in the market.
  We enumerate the main characteristics that distinguish Mezuro from its competitors and why its use can be more advantageus compared to other 
  web-services for source-code analysis.
  Beyond the features a brief discussion of the chosen architecture of the project will be presented, and how this choice is evolving 
  with the progress of the project.
  Finally a use case, illustrated by screenshot, exemplifies the utilization of the tool and how the user can extract information from the analysis.
  \\*
  \textbf{Keywords:} free software, source code analysis, source code metrics, web-service.   
\end{abstract}

\begin{resumo}
  Neste artigo apresentaremos a motivação, as principais características, arquitetura e exemplo de uso do projeto de software livre Mezuro. 
  A motivação por trás do projeto será  apresentada através da problemática da análise de métricas de código, fazendo um breve 
  comparativo com outras soluções disponíveis no mercado. Nos enumeraremos as principais caracteristicas que diferenciam 
  o Mezuro de seus concorrentes e porque o seu uso mostra-se mais vantajoso em comparação a outros serviços web de análise de código fonte.
  Alem das funcionalidades uma breve discussão da arquitetura escolhida para o projeto 
  será apresentada, e como essa escolha está evoluindo com o andamento do projeto. Por fim um caso de uso, ilustrado por captura de tela, exemplifica
  a utilização da ferramenta por um usuário e como este pode extrair informações da análise.
  \\*
  \textbf{Palavras-chave:} software livre, análise de código fonte, métricas de código, serviço web.   
\end{resumo}
